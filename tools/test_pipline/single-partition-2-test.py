...
def reset_directories():
    for dir_path in [test_s_dir, test_l_dir]:
...
def combine_images(small_image_dir, large_image_dir, final_image_dir):
    ...
    for image_dir in [small_image_dir, large_image_dir]:
...
def calculate_final_mIOU(gt_folder, result_folder):
    # write a py file to calculate mIOU first
    command = f"python /path/to/cal_mIOU.py --gt_folder {gt_folder} --result_folder {result_folder}"
    call(command, shell=True)
...
if __name__ == '__main__':
    models_to_run = ["CGNet", 
                     "Changer-mit-b0", "Changer-mit-b1", 
                     "TTP"]  # List of models to process
